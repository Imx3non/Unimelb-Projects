...
import pasur.PointsObserver.PointsTable;
...
public class Pasur
{
    ...
    private RoundStates currRoundState;
    private GameStates prevState;
    private RoundStates prevRoundState;
    ...
    public Pasur(int nPlayers) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException,
        ...
        PointsTable.attach(players[0]);
        PointsTable.attach(players[1]);
    ...
    public void play() throws IOException {
        ...
        while(playGame)
        {
            switch (currState) {
                ...
                case Start:
                    roundOfGame++;
                    gameLogger.write("Round " + roundOfGame + " of the game starts...");

                    isFirstRound = true;
                    reset();
                    currState = GameStates.PlayRound;

                case PlayRound:
                    currRoundState = RoundStates.StartRound;
                    int counter = 1;
                    ...
                    while (!deckHand.isEmpty())
                    {
                        ...
                        for(int i = 0; i < N_HAND_CARDS; i++)
                        {
                            ...
                            for(int j = 0, k = currentStartingPlayerPos; j < nPlayers; j++)
                            {
                                ...
                                if(!cardsToPick.isEmpty())
                                {
                                    ...
                                    PointsTable.notifyAllObservers();
                                    gameLogger.write(player.toString() + " picks " + toString(cardList));
                                    ...
                                    if(isAsur(playedCard, isLastRound))
                                    {
                                        ...
                                        gameLogger.write(player.toString() + " scores a sur");
                                    ...
                                    gameLogger.write(player.toString() + " picks " + toString(cardsToPick));
                        ...
                        if(isLastRound)
                        {
                            ...
                            lastPlayerWhoPickedAcard.calculateScore();

                            if(!poolCards.isEmpty())
                                gameLogger.write(lastPlayerWhoPickedAcard + " picks " + toString(poolCards) + " at the end of this round of game");
                    ...
                    currState = GameStates.endRound;

                case updateScores:
                    /* implement score update here */
                    for(int i = 0; i < nPlayers; i++) {
                        Player player = players[i];
                        player.calculateRoundEndScore();
                    }
                ...
                case endRound:
                    ...
                    gameLogger.write("Round " + roundOfGame + " of the game ends...");
                    currState = GameStates.checkWinner;
...
//        scoreLabel.setText(scoreString);
        gameLogger.write("Total Running Scores: " + scoreString);
    }

    private void dealingOutToPlayers(int currentStartingPlayerPos) throws IOException {
        gameLogger.write("Dealing out to players...");
        ...
        for (int i = 0, k = currentStartingPlayerPos; i < nPlayers; i++)
        {
            ...
            gameLogger.write(player.toString() + " hand: " + toString(player.getHand().getCardList()));
        }
    }

    private void dealingOutToPool() throws IOException {
        gameLogger.write("Dealing out to pool...");
        ...
        gameLogger.write("Pool: " + toString(poolHand.getCardList()));
    }
    ...
    public void setPaused(boolean paused)
    {
        if(paused){
            prevState = currState;
            prevRoundState = currRoundState;
            currState = GameStates.Pause;
            currRoundState = RoundStates.Pause;
        } else{
            currState = prevState;
            currRoundState = prevRoundState;
            prevState = null;
            prevRoundState = null;
        }
        this.paused = paused;
